...
{
    double C = dt / (dx*dx);
    // double del_k = 8 * std::pow(10, 4); // чтобы удовлетворяло условиям
    // C /= del_k;
    C *= (0.01 / C);
...
{
    char name[10];
    sprintf(name, "Prog2v2_%i_Rank%i", threadsNum, rank);
    std::ofstream myfile;
...
void solveHeat(std::vector<double>& U, double a, double b, double Nx, int rank, int sizeProc, double dx, double dt, int NTimeSteps)
{
    ...
    // auto upTo = std::pow(NTimeSteps, sizeProc) * 10;
    // std::cout << "NTimeSteps = " << upTo << "\n";
    auto upTo = NTimeSteps * sizeProc * 100;
...
int main(int argc, char** argv)
{
    ...
    double a = 0, b = 1, t0 = 0, t1 = 0.001;
    int Nx = 50000;
    double dx = (b-a) / Nx, dt = 0.0002;
